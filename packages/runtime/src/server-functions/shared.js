 ...
 * class instances — needs the codec (see `enableRichArguments`). Lives in
 * the shared layer because the client's GET query encoding and the static
 * cache-key derivation must make the same JSON-vs-codec split.
 */
...
// Canonical JSON with sorted object keys. Plain JSON.stringify preserves
// insertion order, so `{ a, b }` and `{ b, a }` — the same call by any
// useful definition — would hash to different static cache keys, and a
// key-order mismatch between the prerender pass and the production client
// would surface as a spurious artifact miss.
function canonicalJSON(value) {
  ...
  for (let i = 0; i < keys.length; i++) {
    out += (i ? "," : "") + JSON.stringify(keys[i]) + ":" + canonicalJSON(value[keys[i]]);
  }
  return out + "}";
}
 ...
 * codec's framed string for everything else — the same JSON-vs-codec split
 * the GET query encoding makes. Both peers derive it independently (the
 * prerendering server when it captures a result, the production client
 * when it fetches the artifact), so the encoding must be deterministic for
 * equal calls: object keys are sorted, and the codec options must match
 * across peers like everywhere else in the protocol.
 */
export async function encodeArgumentsKey(args, codec) {
  if (!args || args.length === 0) return "";
  if (isJSONSafe(args)) return canonicalJSON(args);
...
/**
 * The cache key for a static server function call: `{id}-{hash}` — the
 * function's id plus a hash of the id and the canonical encoded arguments.
 * The prerendering server names artifacts with it and the production
 * client re-derives it to fetch them, so everything here must be
 * deterministic across peers. The key doubles as a filename and a URL path
 * segment: compiled ids are already safe (`{hash}-{n}`), but
 * hand-registered ids can carry characters that cannot ride either (`#`
 * starts a URL fragment, `/` a path segment), so the readable prefix is
 * normalized — the raw id still participates in the hash, which keeps
 * normalized-away distinctions from colliding.
 */
 ...
 * live wire's `text/plain` and avoids the misleading `.json`.
 */
export function staticArtifactName(key) {
  return `${key}.txt`;
}
