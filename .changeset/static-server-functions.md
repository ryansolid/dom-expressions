...
- **`configureServerFunctionsServer({ staticArtifacts })`** is the write-only prerender artifact seam: when configured, every settled static-declared call that returned a plain value is serialized to the framed wire format and handed to `write({ id, key, filename, payload })` — from in-process SSR calls and HTTP GET dispatch alike. Thrown errors, `Response`s, and `ResponseEnvelope`s are never captured (HTTP metadata cannot ride a static file), write failures never break the observed call, and without a writer the branch adds zero overhead.
...
- **Key derivation is deterministic across peers**: the runtime hashes the function id plus canonical encoded arguments with 128 bits of SHA-256, preserving sorted object keys and the configured codec for rich values.
