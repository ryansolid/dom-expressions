...
Deliberately wire-level only — no keying, connection sharing, or value cache (that's the data layer's job: `query(live(fn))` dedupes consumers onto one connection; reactive consumption is already latest-wins). All behavior lives inside the declaration and treeshakes with it: apps that never import `live` carry none of it. Composes with `GET` as `live(GET(fn))`.
